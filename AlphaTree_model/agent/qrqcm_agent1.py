...
from fqf_iqn_qrdqn1.model.alpha_qrdqn import QRDQN
from fqf_iqn_qrdqn1.model.mean import MeanNetwork
...
class QRQCMAgent1(BaseAgent):
    ...
    def __init__(self, env, data_valid, data_test, target, log_dir,
        ...
        super(QRQCMAgent1, self).__init__(
        ...
        self.online_mean_net = MeanNetwork(
            num_actions=self.num_actions,
            dueling_net=dueling_net,
            noisy_net=noisy_net,
            use_tree_lstm=use_tree_lstm
        ...
        self.target_mean_net = MeanNetwork(
            num_actions=self.num_actions,
            dueling_net=dueling_net,
            noisy_net=noisy_net,
            use_tree_lstm=use_tree_lstm
        ...
        if use_tree_lstm and leaf_ids is not None:
            for net in [self.online_net, self.target_net, self.online_mean_net, self.target_mean_net]:
                net.configure_arities(
                    leaf_ids=leaf_ids,
                    unary_ids=unary_ids,
    ...
    def exploit(self, state):
        state = torch.tensor(state, dtype=torch.long, device=self.device).unsqueeze(0)
    ...
    def calculate_mse_loss(self, states, actions, rewards, next_states, dones, weights):
        current_sa_quantiles = evaluate_quantile_at_action(
            self.online_mean_net(states=states),
            actions)
        ...
        with torch.no_grad():
            ...
            next_sa_quantiles = evaluate_quantile_at_action(
                self.target_mean_net(states=next_states),
                next_actions)
